...
| ✅ | **Multi-select and batch actions** | **Library:** ☰ → *Select books…* (or hold a cover → *Select…*) puts the grid/list in selection mode. Tap covers to tick them (thick frame + ☑). ☰ then offers *Select all on this page* / *Select all* / *Deselect all*, KOReader's batch **Reading / On hold / Finished**, **Reset**, **Collections…**, and **Delete…** (one confirmation showing the count and total size). ✕ / Back leaves selection mode first. (Built as a ☰ dialog, the same pattern as KOReader's own file-browser selection, rather than a bottom bar.) Deletion goes through KOReader's own file deletion, so each book's sidecar (progress, highlights) goes with it. The library cache entries and thumbnails are removed too, and Continue Reading/history are updated. **Installed Plugins:** hold a user-installed plugin → *Select plugins to remove…*; tick several, *Remove N selected…*, one confirmation, one restart. Built-in KOReader plugins and this plugin cannot be selected (built-ins can only be *disabled*). Settings of removed plugins are kept; they are unpinned from Home. | Cheap: selection is an in-memory set; the work happens once, when confirmed. |
...
| Status | Feature | How | Cost |
| --- | --- | --- | --- |
| ✅ | **Quick settings bar** (Kindle's swipe-down panel): frontlight, warmth, night mode, Wi-Fi, sleep | Swipe down on Home, or tap the status line (marked ▾), opens a small panel: **Frontlight…** (KOReader's light dialog, warmth included), **Light on/off**, **Night mode**, **Wi-Fi**, **Sleep**. Each sends the same event as KOReader's own gesture action (`ShowFlDialog`, `ToggleFrontlight`, `ToggleNightMode`, `ToggleWifi`, `RequestSuspend`) to the file manager; entries the device lacks are left out. | Cheap; built on demand, nothing runs until tapped. |
